...
// DOM References for Preload Feature
const preloadModal = document.getElementById('preloadModal');
const preloadSupplierInput = document.getElementById('preloadSupplierInput');
const preloadSearchInput = document.getElementById('preloadSearchInput');
const preloadProductsContainer = document.getElementById('preloadProductsContainer');
...
// Preadload Exports
window.openPreloadModal = openPreloadModal;
window.closePreloadModal = closePreloadModal;
window.filterPreloadProducts = filterPreloadProducts;
window.exportPreloadTxt = exportPreloadTxt;
window.importPreloadTxt = importPreloadTxt;
window.applyPreloadToSystem = applyPreloadToSystem;
...
function updateProductStateFromLocalData() {
    if (!dataContext) {
        allProducts = initialProductList.map(product => {
            ...
            return { ...product, id: productId, total_desperdicio_kg: 0, expected_kg: 0, registros: [] };
        });
        return;
    }
    allProducts = initialProductList.map(product => {
        ...
        const expectedKg = productLogs.reduce((sum, log) => {
            const obs = (log.observation || '').toUpperCase();
            if (obs.includes('PRECARGA') || obs.includes('CARGA TXT')) return sum + (log.desperdicio_kg || 0);
            return sum;
        }, 0);

        const totalKg = productLogs.reduce((sum, log) => {
            const obs = (log.observation || '').toUpperCase();
            if (obs.includes('RC') || obs.includes('RECONOCER') || obs.includes('PRECARGA') || obs.includes('CARGA TXT')) return sum;
            return sum + (log.desperdicio_kg || 0);
        }, 0);

        return { ...product, id: productId, total_desperdicio_kg: totalKg, expected_kg: expectedKg, registros: productLogs.sort((a, b) => b.timestamp - a.timestamp) };
    });
}
...
function generateSheetDataForContext(contextName, contextData) {
    ...
    const processedRows = initialProductList.map(prod => {
        ...
        return {
            name: prod.name, logs: sortedLogs, total: sortedLogs.reduce((sum, l) => {
...
function confirmSubtract() {
    ...
    const newLog = {
        ...
        timestamp: Date.now(),
        parentTimestamp: originalLog.timestamp,
    ...
    groups.forEach(group => {
        ...
        group.items.forEach((item, index) => {
            ...
            const processedLogs = sortedRegistros.map(log => {
                ...
                return { kg, obs, isRC, destGroup, singleKg, singleAbbr, bgColor, piecePrefix, originalTimestamp: log.timestamp, parentTimestamp: log.parentTimestamp };
            });

            const pesas = processedLogs.filter(l => !l.isRC);
            const rcs = processedLogs.filter(l => l.isRC);

            const rcsByGroup = {};
            rcs.forEach(rc => {
                if (!rcsByGroup[rc.destGroup]) rcsByGroup[rc.destGroup] = [];
                rcsByGroup[rc.destGroup].push(rc);
            });
            ...
            pesas.forEach(pesa => {
                ...
                // First try to match by exact parent timestamp
                let exactMatchIdx = -1;
                if (rcsByGroup[pesa.destGroup]) {
                    exactMatchIdx = rcsByGroup[pesa.destGroup].findIndex(r => r.parentTimestamp === pesa.originalTimestamp);
                }

                if (exactMatchIdx !== -1) {
                    pairedRc = rcsByGroup[pesa.destGroup].splice(exactMatchIdx, 1)[0];
                }
                // Fallback: take the oldest RC that doesn't have a parent link (legacy support)
                else if (rcsByGroup[pesa.destGroup] && rcsByGroup[pesa.destGroup].length > 0) {
                    const legacyIdx = rcsByGroup[pesa.destGroup].findIndex(r => !r.parentTimestamp);
                    if (legacyIdx !== -1) {
                        pairedRc = rcsByGroup[pesa.destGroup].splice(legacyIdx, 1)[0];
                    } else {
                        // If all have parents but don't match (should be rare), just take the first one
                        pairedRc = rcsByGroup[pesa.destGroup].shift();
                    }
                }

                rows.push({ pesa, rc: pairedRc });
            });

            Object.keys(rcsByGroup).forEach(group => {
                rcsByGroup[group].forEach(rc => {
                    rows.push({ pesa: null, rc: rc });
                });
            });
...
function filterProducts() {
    ...
    filteredProducts.forEach(product => {
        let statusHtml = '';
        if (product.expected_kg > 0) {
            const remaining = product.expected_kg - product.total_desperdicio_kg;
            if (remaining > 0) {
                statusHtml = `
                    <div class="flex flex-col text-right mr-2 justify-center">
                        <span class="text-[9px] text-gray-400 font-bold uppercase tracking-tighter leading-none mb-0.5">Esperado: ${product.expected_kg}kg</span>
                        <span class="text-[12px] font-black text-red-500 leading-none">FALTAN: ${remaining.toFixed(2)}KG</span>
                    </div>
                `;
            } else if (remaining < 0) {
                statusHtml = `
                    <div class="flex flex-col text-right mr-2 justify-center">
                        <span class="text-[9px] text-gray-400 font-bold uppercase tracking-tighter leading-none mb-0.5">Esperado: ${product.expected_kg}kg</span>
                        <span class="text-[12px] text-orange-500 font-black uppercase leading-none">SOBRAN: ${Math.abs(remaining).toFixed(2)}KG</span>
                    </div>
                `;
            } else {
                statusHtml = `
                    <div class="flex flex-col text-right mr-2 justify-center">
                        <span class="text-[9px] text-gray-400 font-bold uppercase tracking-tighter leading-none mb-0.5">Esperado: ${product.expected_kg}kg</span>
                        <span class="text-[12px] text-green-600 font-black uppercase leading-none">COMPLETO</span>
                    </div>
                `;
            }
        } else if (product.total_desperdicio_kg > 0) {
            statusHtml = `<div class="flex flex-col text-right mr-2 justify-center"><span class="text-xs text-indigo-600 font-bold">${product.total_desperdicio_kg}KG</span></div>`;
        }

        const productRow = document.createElement('div');
        productRow.className = 'w-full flex text-sm text-gray-700 border-t border-gray-200 product-row-button items-center p-3 relative';
        productRow.onclick = () => openModal(product);
        productRow.innerHTML = `
            <div class="flex items-center flex-grow min-w-0">
                <img src="${product.imageUrl || DEFAULT_IMAGE_URL}" class="product-image flex-shrink-0" onerror="this.onerror=null; this.src='${DEFAULT_IMAGE_URL}'">
                <span class="font-semibold text-gray-800 break-words">${product.name}</span>
            </div>
            ${statusHtml}
            <svg class="w-5 h-5 text-indigo-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
        `;
...
// ============================================
// === LOGICA PRE-CARGA (RECEPCIÓN TXT) ===
// ============================================
let preloadProductList = [];

function initPreloadList() {
    preloadProductList = allProducts.map(p => ({
        id: p.id,
        name: p.name,
        imageUrl: p.imageUrl,
        quantity: 0
...
preloadSupplierInput.addEventListener('change', (e) => {
    const supplier = e.target.value.trim().toUpperCase();
    if (!supplier) {
        if (preloadProductList.length > 0) {
            preloadProductList.forEach(p => p.quantity = 0);
            filterPreloadProducts();
        }
        return;
    }

    let targetContextDbKey = contextMap["PROVEEDOR"];
    const isContext = contextKeys.includes(supplier);

    if (isContext && supplier !== "PROVEEDOR") {
        targetContextDbKey = contextMap[supplier];
    }

    const store = loadLocalStore();
    let targetData = store[targetContextDbKey] || {};

    if (preloadProductList.length === 0) initPreloadList();

    preloadProductList.forEach(p => p.quantity = 0);

    Object.keys(targetData).forEach(prodId => {
        const logs = targetData[prodId];
        let expected = 0;
        logs.forEach(log => {
            const obs = (log.observation || '').toUpperCase();
            if (!isContext && log.supplier !== supplier) return;

            if (obs.includes('PRECARGA') || obs.includes('CARGA TXT')) {
                expected += (log.desperdicio_kg || 0);
            }
        });
        if (expected > 0) {
            const prod = preloadProductList.find(p => p.id === prodId);
            if (prod) prod.quantity = expected;
        }
    });
    filterPreloadProducts();
});

function renderPreloadProducts(searchTerm = '') {
    preloadProductsContainer.innerHTML = '';

    const term = searchTerm.trim().toLowerCase();
    const filtered = preloadProductList.filter(p => p.name.toLowerCase().includes(term));

    if (filtered.length === 0) {
        preloadProductsContainer.innerHTML = `<div class="text-center text-sm text-gray-500 mt-4">Producto no encontrado.</div>`;
        return;
    }

    filtered.forEach((prod, index) => {
        const isSelected = prod.quantity > 0;
        const row = document.createElement('div');
        row.className = `flex justify-between items-center p-2 mb-1 border rounded-lg transition-colors ${isSelected ? 'bg-orange-50 border-orange-300' : 'bg-white border-gray-200'} shadow-sm`;

        row.innerHTML = `
                    <div class="flex items-center flex-grow min-w-0 pr-2">
                        <img src="${prod.imageUrl || DEFAULT_IMAGE_URL}" class="w-8 h-8 rounded-full object-cover mr-2 flex-shrink-0" onerror="this.onerror=null; this.src='DEFAULT_IMAGE_URL'">
                        <span class="text-xs font-bold text-gray-800 truncate">${prod.name}</span>
                    </div>
                    <div class="flex items-center flex-shrink-0 relative">
                        <input type="number" step="0.01" min="0" data-id="${prod.id}" value="${prod.quantity || 0}" 
                               class="preload-input w-20 text-center text-sm font-bold p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 ${isSelected ? 'text-orange-700 bg-orange-100 border-orange-400' : 'text-gray-700 border-gray-300'}">
                        <span class="text-[10px] font-bold text-gray-400 ml-1">KG</span>
                    </div>
                `;
        preloadProductsContainer.appendChild(row);
    });

    // Set up event listeners for inputs
    const inputs = preloadProductsContainer.querySelectorAll('.preload-input');
    inputs.forEach(input => {
        input.addEventListener('change', (e) => {
            ...
            const prod = preloadProductList.find(p => p.id === id);
            if (prod) {
                prod.quantity = val;
            }

            if (val > 0) {
                e.target.parentElement.parentElement.classList.add('bg-orange-50', 'border-orange-300');
                e.target.parentElement.parentElement.classList.remove('bg-white', 'border-gray-200');
                e.target.classList.add('text-orange-700', 'bg-orange-100', 'border-orange-400');
                e.target.classList.remove('text-gray-700', 'border-gray-300');
            } else {
                e.target.parentElement.parentElement.classList.remove('bg-orange-50', 'border-orange-300');
                e.target.parentElement.parentElement.classList.add('bg-white', 'border-gray-200');
                e.target.classList.remove('text-orange-700', 'bg-orange-100', 'border-orange-400');
                e.target.classList.add('text-gray-700', 'border-gray-300');
            }
        });

        // Limpiar 0 al hacer foco
...
function filterPreloadProducts() {
    renderPreloadProducts(preloadSearchInput.value);
}

function openPreloadModal() {
    hamburgerDropdown.classList.add('hidden');
    preloadSupplierInput.value = '';
    preloadSearchInput.value = '';
    initPreloadList();
    renderPreloadProducts();

    const datalist = document.getElementById('preloadSupplierOptions');
    if (datalist) {
        datalist.innerHTML = '';
    ...
    preloadModal.classList.remove('hidden');
}

function closePreloadModal(event) {
    if (event && event.target.id !== 'preloadModal') return;
    preloadModal.classList.add('hidden');
}

function exportPreloadTxt() {
    const supplier = preloadSupplierInput.value.trim().toUpperCase();
    if (!supplier) {
        showToast("Debe ingresar el nombre del PROVEEDOR", "error");
        return;
    }
    const activeItems = preloadProductList.filter(p => p.quantity > 0);
    if (activeItems.length === 0) {
        showToast("No hay cantidades cargadas para exportar", "error");
        return;
    }

    const exportData = {
        type: "PRELOAD_DATA_V1",
        supplier: supplier,
    ...
    const dateObj = new Date();
    const dateStr = dateObj.toLocaleDateString('es-VE').replace(/\//g, '-');
    const fileName = `PRECARGA_${supplier.replace(/\s+/g, '_')}_${dateStr}.txt`;
    ...
    showToast("Archivo TXT exportado exitosamente", "success");
}

function importPreloadTxt(event) {
    ...
    reader.onload = function (e) {
        try {
            ...
            if (data.type !== "PRELOAD_DATA_V1") {
                showToast("El archivo no tiene el formato correcto", "error");
                return;
            }

            preloadSupplierInput.value = data.supplier || "";
            ...
            data.items.forEach(item => {
                const prod = preloadProductList.find(p => p.id === item.id || p.name === item.name);
                if (prod) {
            ...
            preloadSearchInput.value = '';
            renderPreloadProducts();
            showToast("Carga importada correctamente", "success");

        } catch (error) {
            console.error(error);
            showToast("Error al leer el archivo TXT", "error");
        }

        // Clear the input so same file can trigger change event again
        event.target.value = "";
    };
    reader.readAsText(file);
}

function applyPreloadToSystem() {
    const supplier = preloadSupplierInput.value.trim().toUpperCase();
    if (!supplier) {
        showToast("Debe ingresar el PROVEEDOR para aplicar al sistema", "error");
        return;
    }

    const activeItems = preloadProductList.filter(p => p.quantity > 0);
    if (activeItems.length === 0) {
        showToast("No hay cantidades cargadas", "error");
        return;
    }

    // Target Context is PROVEEDOR
    let targetContextDbKey = contextMap["PROVEEDOR"];
    let entrySupplier = supplier;

    // Si el texto introducido coincide con una sede existente
    const isContext = contextKeys.includes(supplier);
    if (isContext && supplier !== "PROVEEDOR") {
        targetContextDbKey = contextMap[supplier];
        entrySupplier = null; // null representa la misma sede
    }
    ...
    Object.keys(targetData).forEach(prodId => {
        targetData[prodId] = targetData[prodId].filter(log => {
            const obs = (log.observation || '').toUpperCase();
            const isPreloadLog = (obs.includes('PRECARGA') || obs.includes('CARGA TXT'));
            if (!isContext && log.supplier !== supplier) return true; // not this supplier
            if (isPreloadLog) return false; // remove old preloads
            return true;
        });
    });

    activeItems.forEach(item => {
        if (!targetData[item.id]) targetData[item.id] = [];

        const newEntry = {
            desperdicio_kg: item.quantity,
            timestamp: Date.now(),
            responsible: currentResponsible || 'SIN ASIGNAR',
            supplier: entrySupplier,
            observation: 'Destino: PRECARGA'
        };

        targetData[item.id].unshift(newEntry);
    });

    store[targetContextDbKey] = targetData;
    localStorage.setItem(LOCAL_STORAGE_KEY, JSON.stringify(store));

    // Also make sure supplier is in saved suppliers list
    const suppIndex = savedSuppliers.indexOf(supplier);
    if (suppIndex > -1) {
        savedSuppliers.splice(suppIndex, 1);
    }
    savedSuppliers.unshift(supplier);
    localStorage.setItem(SUPPLIERS_LIST_KEY, JSON.stringify(savedSuppliers));
    ...
    // Set current active to context to see the result
    if (isContext && supplier !== "PROVEEDOR") {
        activateContext(supplier, supplier);
    } else {
        currentSupplier = supplier;
        activateContext("PROVEEDOR", `PROVEEDOR: ${supplier}`);
    }

    closePreloadModal();
    showToast("Mercancía cargada al proveedor exitosamente", "success");
}
