        ...
        let currentActiveKey = "SELECCIONAR";
        let currentActiveLabel = "SELECCIONE GRUPO O PROVEEDOR";
        ...
        // --- RECONOCIMIENTO DE VOZ PROVEEDOR ---
        function startSupplierVoiceRecognition() {
            const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
            if (!SpeechRecognition) {
                showToast("Su navegador no soporta reconocimiento de voz", "error");
                return;
            }
            ...
            const originalPlaceholder = supplierNameInput.placeholder;
            supplierNameInput.placeholder = "Escuchando...";
            
            recognition.start();

            recognition.onresult = function(event) {
                const transcript = event.results[0][0].transcript;
                supplierNameInput.value = transcript.toUpperCase();
                supplierNameInput.placeholder = originalPlaceholder;
                handleSupplierInput(transcript); // Actualizar estado o buscar si fuera necesario
            };
