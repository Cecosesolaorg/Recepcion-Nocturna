        ...
        const PRODUCT_NOTES_KEY = 'wasteProductNotes'; // NUEVA KEY
        ...
        let dataContext = contextMap["SAN ROQUE"]; 
        let viewMode = 'entry'; 
        
        // VARIABLES PARA RECORDAR LA SELECCIÓN ANTERIOR
        let currentActiveKey = "SAN ROQUE";
        let currentActiveLabel = "SAN ROQUE";
        ...
        function updateProductStateFromLocalData() {
            allProducts = initialProductList.map(product => {
                ...
                const totalKg = productLogs.reduce((sum, log) => sum + (log.desperdicio_kg || 0), 0);
        ...
        function renderSupplierSuggestions(filterText = '') {
            ...
            if (cleanFilter === '') {
                matches = savedSuppliers.slice(0, 15);
            } else {
                matches = savedSuppliers.filter(s => s.startsWith(cleanFilter));
            }

            if (matches.length > 0) {
                const title = document.createElement('p');
                title.className = "w-full text-xs text-gray-500 font-bold mb-1";
                title.textContent = cleanFilter === '' ? "RECIENTES:" : "COINCIDENCIAS:";
                supplierSuggestions.appendChild(title);

                matches.forEach(name => {
                    const btn = document.createElement('button');
                    btn.className = "bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-3 py-1 text-xs font-bold hover:bg-purple-100 transition mb-2 mr-2";
                    btn.textContent = name;
                    btn.onclick = () => useSupplierSuggestion(name);
                    supplierSuggestions.appendChild(btn);
                });
            } else if (cleanFilter !== '') {
                 const msg = document.createElement('p');
                 msg.className = "w-full text-xs text-gray-400 italic";
                 msg.textContent = "No hay proveedores guardados con ese nombre.";
                 supplierSuggestions.appendChild(msg);
            }
        }
        ...
        function generateSheetDataForContext(contextName, contextData) {
            ...
            const processedRows = initialProductList.map(prod => {
                ...
                return { name: prod.name, logs: sortedLogs, total: sortedLogs.reduce((sum, l) => sum + l.desperdicio_kg, 0) };
            });
        ...
        function selectMenuItem(event) {
            ...
            if (selectedKey === 'PROVEEDOR') {
                menuDropdown.classList.add('hidden');
                // Si ya hay un proveedor seleccionado, activamos el contexto directament
                // Si no, pedimos nombre
                ...
                if (!isRC) {
                    btnHtml = `
                        <button onclick="openSubtractModal(${log.timestamp}, '${productId}', '${contextKey}', ${log.desperdicio_kg})" 
                            class="bg-indigo-600 text-white font-bold text-xs px-2 py-1.5 rounded hover:bg-indigo-700 uppercase tracking-wide mt-1">
                            RECONOCER
                        </button>
                    `;
                } else {
                    btnHtml = `<span class="text-xs font-bold text-orange-500 uppercase border border-orange-200 bg-orange-50 px-2 py-1 rounded mt-1 inline-block">RECONOCIDO</span>`;
                }
        ...
        function confirmClearData() {
            ...
            // NUEVO: Borrar también la lista de proveedores
            ...
            showToast("Datos y lista de proveedores borrados.", "success");
        }
            ...
            contextKeys.forEach((cKey, cIndex) => { 
                ...
                initialProductList.forEach(prod => {
                    ...
                    if (cKey === "PROVEEDOR") {
                        ...
                        const totalKg = logs.reduce((sum, l) => sum + (l.desperdicio_kg || 0), 0);
